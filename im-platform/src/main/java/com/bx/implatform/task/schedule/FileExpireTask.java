...
import com.bx.implatform.thirdparty.MinioStorageService;
    ...
    public void run() {
        ...
        while (true) {
            for (FileInfo fileInfo : files) {
                ...
                if (minioStorageService.isExist(bucket, path, fileNme)) {
                    if (!minioStorageService.remove(bucket, path, fileNme)) {
                        // 删除失败，不再往下执行
