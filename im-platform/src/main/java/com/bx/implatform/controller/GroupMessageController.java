...
public class GroupMessageController {
    ...
    @RepeatSubmit
    @PostMapping("/update")
    @Operation(summary = "编辑群聊消息", description = "编辑群聊消息")
    public Result<GroupMessageVO> update(@Valid @RequestBody GroupUpdateMessageDTO vo) {
        return ResultUtils.success(groupMessageService.updateMessage(vo));
    }
    ...
    @DeleteMapping("/manage/recall/{id}")
    @Operation(summary = "管理撤回成员消息", description = "管理撤回成员消息")
    public Result<GroupMessageVO> manageRecallMessage(@NotNull(message = "消息id不能为空") @PathVariable Long id) {
        return ResultUtils.success(groupMessageService.manageRecallMessage(id));
    }
    ...
    public Result<Void> readedMessage(@RequestParam Long groupId) {
