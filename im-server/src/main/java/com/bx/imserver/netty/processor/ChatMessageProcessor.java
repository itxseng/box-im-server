...
public class ChatMessageProcessor extends AbstractMessageProcessor<IMRecvInfo> {
        ...
        try {
            ...
            deviceIds.forEach(deviceId -> {
                ...
                if (!Objects.isNull(channelCtx)) {
                    ...
                    sendInfo.setCmd(IMCmdType.CHAT_MESSAGE.code());
    ...
    private void sendResult(IMRecvInfo recvInfo, IMSendCode sendCode) {
        if (recvInfo.getSendResult()) {
            ...
            // 推送到结果队列
            String key = StrUtil.join(":", IMRedisKey.IM_RESULT_CHAT_QUEUE, recvInfo.getServiceName());
