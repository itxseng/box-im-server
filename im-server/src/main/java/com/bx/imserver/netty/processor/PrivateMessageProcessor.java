...
public class PrivateMessageProcessor extends AbstractMessageProcessor<IMRecvInfo> {
    ...
    public void process(IMRecvInfo recvInfo) {
        ...
        log.info("接收到私聊消息，发送者:{},接收者:{}，内容:{},设备ID：{}", sender.getId(), receiver.getId(), recvInfo.getData(),sender.getDeviceId());
        try {

            String key = StrUtil.join(":", IMRedisKey.IM_USER_DEVICE_ID, receiver.getId(),receiver.getTerminal());
            Set<Object> deviceIds = redisMQTemplate.opsForSet().members(key);
            //把发送者设备id排除 发送者接收者一样时
            if (sender.getId().equals(receiver.getId())){
                if (StringUtils.hasText(sender.getDeviceId())){
                    assert deviceIds != null;
                    log.info("排除发送设备id:{}",sender.getDeviceId());
                    deviceIds.remove(sender.getDeviceId());
                }
            }
            assert deviceIds != null;
            deviceIds.forEach(deviceId -> {
                ChannelHandlerContext channelCtx = UserChannelCtxMap.getChannelCtx(receiver.getId(), receiver.getTerminal(),deviceId.toString());
                if (!Objects.isNull(channelCtx)) {
                    ...
                    String deviceKey = String.join(":", IMRedisKey.IM_USER_DEVICE_ID, receiver.getId().toString(), receiver.getTerminal().toString());
